...
//            }
    ...
    override fun getProductToppings(): Flow<List<Topping>> = flow {
        try {
            val snapshot = firestore.collection("toppings").get().await()
            val toppings = snapshot.documents.mapNotNull {
                it.toObject(ToppingDto::class.java)?.copy(id = it.id)
            }

            emit(toppings.map { it.toTopping() })
        } catch (e: Exception) {
            throw e
        }
    }
...
//            val collection = firestore.collection("toppings")
//
//            toppingList.forEach { product ->
//                batch.set(
//                    collection.document() ,
//                    hashMapOf(
